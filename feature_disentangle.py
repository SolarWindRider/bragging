...
if __name__ == '__main__':
    ...
    for idx, inputs in enumerate(train_loader):
        ...
        if total_step >= ceil(2838 / conf.BATCHSIZE * conf.EPOCHS * 0.7):
            if total_step % 1000 == 0:  # 加载测试
                ...
                for average in ["micro", "macro"]:
                    ...
                    print(f"g_{conf.MODLENAME}_{total_step}_{average}\nP: {P:6.3%}, R: {R:6.3%}, F1: {F1:6.3%}")
                    logging.info(f"g_{conf.MODLENAME}_{total_step}_{average}\nP: {P:6.3%}, R: {R:6.3%}, F1: {F1:6.3%}")
                    if average is "macro" and F1 > best_macroF1:
                        best_macroF1 = F1
                        writer.add_scalar("best_macroF1", best_macroF1, total_step)
